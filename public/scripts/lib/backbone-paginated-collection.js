...
function updatePagination() {
  ...
  this._collection.reset(this.usersCollection().slice(pages[0], pages[1]));
}

function updateNumPages() {
  var currentNumPages = this._totalPages;
  var length = this.usersCollection().length;
...
function onAddRemove(model, collection, options) {
  ...
  var toAdd = difference(this.usersCollection().slice(start, end), this._collection.toArray());
  var toRemove = difference(this._collection.toArray(), this.usersCollection().slice(start, end));
  ...
  if (toAdd) {
    this._collection.add(toAdd, {
      at: this.usersCollection().indexOf(toAdd) - start
    });
  }
}
